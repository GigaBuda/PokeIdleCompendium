...
const lm=raw.match(/(?:Lv\.?|Nivel|Nível)\s*(\d+)/i),qm=raw.match(/(?:Qualidade|Raridade)[^\d]{0,20}(?:×|x)\s*(\d+(?:[.,]\d+)?)/i)||raw.match(/(?:×|x)\s*(\d+[.,]\d+)/),im=raw.match(/IV\s*(\d+)\s*\/\s*(\d+)/i),pm=raw.match(/(?:Poder|Power)\s*[:\-]?\s*(\d+)/i);
...
#${CFG.panelId} .jp-meta-box{background:linear-gradient(180deg,#142b30,#102124);border:1px solid #334d4c;border-radius:4px;padding:6px 6px;text-align:center;min-width:0;display:flex;flex-direction:column;align-items:center;justify-content:center}
#${CFG.panelId} .jp-label{display:block;width:100%;font-size:9px;line-height:11px;text-transform:uppercase;color:#78918e;margin-bottom:3px;text-align:center}
#${CFG.panelId} .jp-value{display:block;width:100%;font-size:16px;line-height:18px;font-weight:900;white-space:nowrap;text-align:center}
...
#${CFG.panelId} .jp-rating-title{font-size:17px;font-weight:900;color:#f1c644}
...
#${CFG.panelId} .jp-input{width:100%;padding:4px 7px;background:#0b181a;border:1px solid #334c4a;border-radius:3px;color:#e3eeea;font-size:11px;height:24px}
...
const closeButton=box.querySelector("[data-panel-close]");if(closeButton&&!closeButton.dataset.bound){closeButton.dataset.bound="1";closeButton.addEventListener("click",()=>{box.style.display="none"})}
...
const saved=localStorage.getItem(CFG.storageKey);if(saved){try{const z=JSON.parse(saved);if(z.w&&z.h){box.style.width=Math.max(420,Math.min(innerWidth-12,z.w))+"px";box.style.height=Math.max(520,Math.min(innerHeight-12,z.h))+"px"}if(Number.isFinite(z.x)&&Number.isFinite(z.y)){box.style.transform="none";box.style.left=Math.max(0,Math.min(innerWidth-box.offsetWidth,z.x))+"px";box.style.top=Math.max(0,Math.min(innerHeight-box.offsetHeight,z.y))+"px"}}catch{}}
...
window.addEventListener("resize",()=>{const box=document.getElementById(CFG.panelId);if(!box)return;const maxW=Math.max(420,innerWidth-12),maxH=Math.max(520,innerHeight-12);if(box.offsetWidth>maxW)box.style.width=maxW+"px";if(box.offsetHeight>maxH)box.style.height=maxH+"px"});
})();