...
(function () {
  ...
  function creatureBase(c, key) {
    ...
    for (const container of containers) {
      ...
      for (const name of fallbackNames[key] || []) {
        if (container[name] != null && Number.isFinite(Number(container[name]))) return Number(container[name]);
      }
    }
  ...
  async function loadCreatures() {
    const urls = ["/game/creatures.json", "/data/creatures.json"];
  ...
  function spriteUrls(creature, pokemon) {
    ...
    return { anim: `${base}/versions/generation-v/black-white/animated/${folder}${id}.gif`, still: `${base}/${folder}${id}.png` };
  }
  ...
  function createPanel() {
    ...
    panel.innerHTML = `<div class="pil-head" id="pil-drag"><div class="pil-title">🧮 PokeIdleLab IV</div><button class="pil-close">×</button></div><div class="pil-body" id="pil-content"></div>`;
